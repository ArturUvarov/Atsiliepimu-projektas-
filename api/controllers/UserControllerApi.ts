...
//   // Gaumane visus vartotojus iš schemos
//   const users = await TagsRouter.getAll(fields);
...
//   const user = await TagsRouter.getById(req.params.id, fields);
...
//   const user = await TagsRouter.getByEmail(data.email);
//   if (user) {
...
//   const result = await TagsRouter.insert(data);
...
//   const user = await TagsRouter.getById(req.params.id);
...
//   // tikriname el. paštą
//   const user_for_email = await TagsRouter.getByEmail(data.email);
...
//   // siunčiame į DB per modelį
//   const result = await TagsRouter.update(data, user.id);
...
//   const result = await TagsRouter.delete(user.id);
